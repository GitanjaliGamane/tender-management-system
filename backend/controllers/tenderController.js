...
const selectWinner = async (req, res) => {
  ...
  try {
    ...
    // ✅ CHANGE 3: Block if winner already selected
    if (tender.status === 'completed' || tender.winnerCompanyId) {
      return res.status(400).json({
        message: 'A winner has already been selected for this tender. Only one winner is allowed.',
      });
    }
 
    // Find the winning company
    const company = await User.findById(companyId);
    if (!company) {
      return res.status(404).json({ message: 'Company not found. Please check the Company ID.' });
    }
 
    // Update tender with winner and mark as completed
    const updatedTender = await Tender.findByIdAndUpdate(
      req.params.id,
      {
        winnerCompanyId: company._id,
    ...
    res.json({ message: 'Winner selected successfully', tender: updatedTender });
  } catch (error) {
    res.status(500).json({ message: 'Server error: ' + error.message });
  }
};
